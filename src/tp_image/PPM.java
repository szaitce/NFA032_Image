...
public class PPM {
	 ...
	 * @param t :
	 */
	public static Pixel[][] lireFichier(String cheminFichier) {
		...
		Pixel[][] t = new Pixel [nbLig][nbCol];
		int k = 3, nbPixParLigne = 0, numLigne = 0, numCol = 0;
		while (k < tabValeurs.length) {
			t[numLigne][numCol].setRouge(tabValeurs[k]);
			t[numLigne][numCol].setVert(tabValeurs[k+1]);
			t[numLigne][numCol].setBleu(tabValeurs[k+2]);
			t[numLigne][numCol].setAlpha(255);
			k +=3;
			nbPixParLigne++;
	 ...
	 * @param picture :
	 */
	public static void ecrireFichier(String cheminFichier, Pixel[][] picture) {
		int nbLig = picture.length;
		int nbCol = picture[0].length;
		...
		try {
			...
			for(int i=0; i<nbLig; i++) {
				for (int j = 0; j < nbCol; j++) {
					ecrivain.write(""+picture[i][j].getRouge()+"\n");
					ecrivain.write(""+picture[i][j].getVert()+"\n");
					ecrivain.write(""+picture[i][j].getBleu()+"\n");
				}
			}
			ecrivain.close();
		}
